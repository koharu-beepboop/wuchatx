...
s.b=s.b.slice(i+2)
if(c=="NICK"){let n=a[1];if(N.has(n)&&N.get(n)!==s){s.write(`:server 433 * ${n} :Nickname is already in use\r\n`)}else{let o=s.n;s.n=n;if(o){N.delete(o);N.set(s.n,s)
...
}},close(s){N.delete(s.n);if(s.h!=null)for(let h of s.h){let ch=C.get(h);if(ch){ch.delete(s);for(let x of ch)x.write(`:${s.n}!${s.u||s.n}@0 QUIT :gone\r\n`);if(!ch.size)C.delete(h)}}}}})
